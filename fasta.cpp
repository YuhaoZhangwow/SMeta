...
struct Sequence{
	...
	void statistic(){
		int LEN=SEQ.size();
		char placement[4]={'A','C','G','T'};
		
		#pragma omp parallel for
		for(int i=0;i<LEN;i++){
			SEQ[i]=toupper(SEQ[i]);
			if(SEQ[i]=='N'){
				SEQ[i]=placement[rand()%4];
			}
		}
...
//				if(debug)cerr<<"this is checkpoint7\n";
	...
	void detect_relation(vector<Sequence> META, double cutoff){
		...
		cout<<'\n'<<"Progress bar[O represent 10% complete]:\n";
		
		auto compare=[](const pair<int,double>&a, const pair<int,double>&b){return a.second>b.second;};
		
		for(auto it1=META.begin();it1!=META.end();it1++,icount++){
			
			if(icount%META_SIZE==0&&icount){
				cerr<<"O";
			}
			
			priority_queue<pair<int, double>, vector<pair<int, double>>, decltype(compare)> min_heap(compare);
...
//				if(min_heap.size()==MAX_EDGE_IN_SEG&&min_heap.top().second>0.98){
//					it1->category=category;
//					it1->SINGLE_OR_NOT=1;
//					break;
//				}
				
				if(flag>=number_of_seq*1.0*4/3){
					it1->category=category;
					it1->SINGLE_OR_NOT=1;
					oanji=1;
					break;
...
//						buildgraph(start+i,it1->number,similarity);
						...
						if(min_heap.size()<MAX_EDGE_IN_SEG||(min_heap.size()==MAX_EDGE_IN_SEG&&similarity>min_heap.top().second)){
							if(min_heap.size()==MAX_EDGE_IN_SEG)min_heap.pop();
							min_heap.push(make_pair(start+i,similarity));
						}
...
//						buildgraph(it1->number,start+i,similarity);
			...
			while(!min_heap.empty()){
				...
				pair<int,double>edge=min_heap.top();
				buildgraph(it1->number,edge.first,edge.second);
				min_heap.pop();
			}
		}
	}
...
std::vector<Eigen::MatrixXd> matrixlizationEigen(const std::vector<Sequence>& T,int shuffleornot) {
    ...
    // 每个矩阵的行数限制
    const int maxRowsPerMatrix = 20000;
    // 计算需要多少个矩阵来存储所有行
    int numMatrices = (T.size() + maxRowsPerMatrix - 1) / maxRowsPerMatrix;
    // 创建一个 vector 来存储所有的 Eigen 矩阵
    std::vector<Eigen::MatrixXd> matrices(numMatrices);
    // 为每个矩阵分配适当的行数
    for (int m = 0; m < numMatrices; ++m) {
        // 确定当前矩阵的行数
        int numRows = std::min(maxRowsPerMatrix, static_cast<int>(T.size()) - m * maxRowsPerMatrix);
        // 创建矩阵
        matrices[m] = Eigen::MatrixXd(numRows, 136);
        // 填充矩阵
        for (int i = 0; i < numRows; ++i) {
            // 计算 T 中的索引
            int index = m * maxRowsPerMatrix + i;
            // 使用 Eigen 的 Map 类来直接映射内存
            matrices[m].row(i) = Eigen::Map<const Eigen::RowVectorXd>(T[idx[index]].TNF, 136);
        }
    }
    return matrices;
}
...
//		cerr<<"sq: "<<line<<'\n';
    ...
    if(debug)cerr<<"total sequence number is: "<<number_of_sq<<'\n';
    inputFile.close();
    return DNA_DATA;
}
...
void gen_tnf_graphEigen(double cutoff, int nobs, const Eigen::MatrixXd& matrix,int which_matrix_i,int which_matrix_j,int batch_size) {
	...
	int nobs1=matrix.rows();
	int nobs2=matrix.cols();
//#pragma omp parallel for schedule(dynamic, 1) proc_bind(spread) reduction(merge_int: from) reduction(merge_int: to) reduction(merge_double: sTNF) 
//#pragma omp parallel for schedule(dynamic, 1) reduction(merge_int: from) reduction(merge_int: to) reduction(merge_double: sTNF) 
    for ( int ii= 0; ii < nobs1; ii+=TILE ){
        std::vector<std::priority_queue<std::pair<int, double>, std::vector<std::pair<int, double> >, CompareEdge> > edges(TILE);

        for ( int jj= 0; jj < nobs2; jj+=TILE ) {
            for (int i = ii; i < ii+TILE && i<nobs1; ++i){
                int que_index = i-ii;
                for (int j = jj; j < jj+TILE && j < nobs2; ++j) {
                    if (i == j && which_matrix_i==which_matrix_j)
                        continue;
                    double sTNF = matrix(i, j);
                    if (sTNF > cutoff &&
                        (edges[que_index].size() < maxEdges || (edges[que_index].size() == maxEdges && sTNF > edges[que_index].top().second))) {
...
//	}
	...
	if(debug)cout<<"\nnEdges->"<<nEdges<<'\n';
	if(debug)cout<<"Progress bar for LPA algorithm [X represents 10% completed]\n";
...
//	while(1);
    ...
    for (int i=0;i<argc;i++) {
        ...
        if ((arg=="-meta"||arg=="-m")&&i+1<argc) {
            fileF=argv[i+1];
            i++;  
        } 
		else if((arg=="-single"||arg=="-s")&&i+1<argc) {
            fileS=argv[i+1];
            i++;  
        }
        else if(arg=="-h"||arg=="-help"){
        	cerr << "Usage: fasta.exe -meta <metadata_path> -single <sequence_path> [-options]"<<endl<<endl;
        	cerr << "	-meta/-m <string>: Path to the metadata file [Mandatory]." << endl <<endl;
			cerr << "	-single/-s <string>: Path to the single sequence directory [Mandatory]." << endl <<endl;
			cerr << "	-slice <number>: Number of segment trees per single cell sequence (default: 2)." << endl <<endl;
			cerr << "	-maxedges <number>: Maximum number of edges built for one metagenome sequence (default: 200)." << endl <<endl;
			cerr << "	-maxedgeseg <number>: Maximum number of edges in segment tree function (default: 50)." << endl <<endl;
			cerr << "	-debug/-d <1 or 0>: Debug mode, enable with 1 and disable with 0 (default: 1)." << endl <<endl;
			cerr << "	-maxp <double>: Maximum threshold for the proportion of cover (default: 0.95)." << endl <<endl;
			cerr << "	-time/-t/-T <1 or 0>: Enable timing in process (default: 0)." << endl <<endl;
		}
		else if(arg=="-slice"&&i+1<argc) {
            COVER_NUMBER=stoi(argv[i+1]);
            i++;  
        }
        else if(arg=="-maxedges"&&i+1<argc) {
            maxEdges=stoi(argv[i+1]);
            i++;  
        }
        else if(arg=="-maxedgeseg"&&i+1<argc) {
            MAX_EDGE_IN_SEG=stoi(argv[i+1]);
            i++;  
        }
        ...
        else if(arg=="-maxp"&&i+1<argc) {
            MAX_EDGE_IN_SEG=stod(argv[i+1]);
            i++;  
        }
        else if((arg=="-time"||arg=="-t"||arg=="-T")&&i+1<argc) {
            timing_or_not=stoi(argv[i+1]);
            i++;  
        }
    }
...
// this is deprecated, DO NOT USE
    ...
    cerr << "MetaFile specified after -f: " << fileF << std::endl;
	vector<Sequence> Meta=Read_sequence_func(fileF);
	
	if(timing_or_not){
		auto end_time = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::seconds>(end_time - start_time);
		cerr << "Elapsed time in reading: " << duration.count() << " seconds" << endl <<endl;
		start_time = end_time;
	}
	
    cerr << "SingleFile specified after -s: " << fileS << std::endl;
...
//	if(debug){
//		AVG_LENGTH=20;// this code is only for special debug!!!!!  delete after debugging!!!!!
//	}
...
//		if(debug)cerr<<"this is checkpoint1\n";
		
		SEGMENT_TREE T(temp);
		
//		if(debug)cerr<<"this is checkpoint2\n";
...
//	}
	...
	if(timing_or_not){
		auto end_time = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::seconds>(end_time - start_time);
		cerr << "\nElapsed time in random matrixilization : " << duration.count() << " seconds" << endl;
		start_time = end_time;
	}
...
//	std::thread thread1([&]() {	
	...
	for(int i=0;i<how_many;i++){
		if(debug)cout<<"Working on No."<<i<<" submatrix\n";
		Eigen::MatrixXd MatrixCosMetaMeta=cosangleEigen(MatrixMeta[i],MatrixMeta[i]);
		pTNF+=gen_tnf_graph_sampleEigen(maxP,false,MatrixCosMetaMeta.rows(),MatrixCosMetaMeta);
//		Eigen::MatrixXd MatrixCosMetaMeta=cosangleEigen(MatrixMeta,MatrixMeta);
	...
	for(int i=0;i<how_many;i++){
		...
		for(int j=i;j<how_many;j++){
			if(debug)cout<<"*";
			Eigen::MatrixXd MatrixCosMetaMeta=cosangleEigen(MatrixMeta[i],MatrixMeta[j]);
			gen_tnf_graphEigen(pTNF/1000.0,Meta.size(),MatrixCosMetaMeta,i,j,20000);
		}
	}
	
	if(debug)cerr<<"\ncomplete cosine matrix calculation\n";
	
//	int pTNF=gen_tnf_graph_sampleEigen(maxP,false,Meta.size(),MatrixCosMetaMeta);
	if(timing_or_not){
		auto end_time = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::seconds>(end_time - start_time);
		cerr << "Elapsed time in building edges inside metagenome : " << duration.count() << " seconds" << endl <<endl;
		start_time = end_time;
	}
	
	if(debug)cerr<<"edges from matrix are->"<<e.size()<<"\n\n";
...
//	thread thread2([&](){
    ...
    for (int i = 0; i < SCS_SEGMENT_TREE.size(); ++i) {
        if (debug) {
            {
...
//	});
	...
	if(timing_or_not){
		auto end_time = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::seconds>(end_time - start_time);
		cerr << "\nElapsed time in LPA: " << duration.count() << " seconds" << endl<<endl;
		start_time = end_time;
	}
	...
	if(timing_or_not){
		auto end_time = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::seconds>(end_time - PROGRAM_TIME);
		cerr << "\nTotal time: " << duration.count() << " seconds" << endl;
	}
...
// fasta.exe -meta C:\Users\okura\Downloads\testdata\2019_TBMTW-1.contigs.fa -single C:\Users\okura\Downloads\euk_test\euk_test -threshold 30

//fasta.exe -meta C:\Users\okura\Downloads\testdata\metagenome.fasta -single C:\Users\okura\Downloads\euk_test\empty -threshold 30 -folder C:\Users\okura\Downloads\testdata\statis
